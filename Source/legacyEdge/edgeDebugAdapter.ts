...
export class EdgeDebugAdapter extends CoreDebugAdapter {
	...
	protected doAttach(
		...
		return super
			...
			.then(async () => {
				...
				const userAgentPromise = this.globalEvaluate({
	...
	public async evaluate(
		...
		return super.evaluate(args).then((evalResponseBody) => {
			if (
				evalResponseBody.type === "Object" &&
				evalResponseBody.result === "" &&
				evalResponseBody.indexedVariables === undefined
			) {
	...
	public createPrimitiveVariable(
		name: string,
