...
import { defaultTargetFilter, getBrowserPath, getTargetFilter } from "./utils";
...
export class EdgeConfigurationProvider
		...
		if (config.request === "attach") {
			...
			await protocolDetection
				...
				.then((detectedBrowserProtocol) => {
					if (
						protocolDetection
							.extractBrowserProtocol(detectedBrowserProtocol)
							.indexOf("Chrome") > -1
					) {
						config.type = "msedge";
					}
