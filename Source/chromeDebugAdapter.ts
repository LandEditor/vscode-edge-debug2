...
interface IExtendedInitializeRequestArguments
...
export class ChromeDebugAdapter extends CoreDebugAdapter {
	...
	public launch(
		...
		return super.launch(args, telemetryPropertyCollector).then(async () => {
			...
			runtimeExecutable =
				runtimeExecutable || utils.getBrowserPath("stable");
			...
			const chromeEnv: coreUtils.IStringDictionary<string> =
				args.env || null;
			...
			if (
				...
				(typeof args.userDataDir === "undefined" &&
					!args.runtimeExecutable)
			) {
			...
			this._chromeProc = await this.spawnChrome(
				runtimeExecutable,
				chromeArgs,
				chromeEnv,
				chromeWorkingDir,
	...
	protected onFrameNavigated(params: Crdp.Page.FrameNavigatedEvent): void {
		if (this._userRequestedUrl) {
			...
			if (
				url === requestedUrlNoAnchor ||
				decodeURI(url) === requestedUrlNoAnchor
			) {
				// 'http://localhost:1234/test%20page' will use the not decoded version, 'http://localhost:1234/test page' will use the decoded version
			...
			.then(async () => {
				...
				this.globalEvaluate({
					...
					.then(() => {
						...
						const cacheDisabled =
							typeof configDisableNetworkCache === "boolean"
				...
				const versionInformationPromise =
					this.chrome.Browser.getVersion().then(
						(response) => {
							const properties = {
								"Versions.Target.CRDPVersion":
						...
						(err) => {
							...
							const properties = {
								"Versions.Target.NoUserAgentReason":
									"Error while retriving target user agent",
				...
				try {
					if (this._breakOnLoadHelper) {
						...
						const browserVersion = (
							await this._chromeConnection.version
						).browser;
	...
	private async spawnChrome(
		...
		if (platform === coreUtils.Platform.Windows && shouldLaunchUnelevated) {
			...
			platform === coreUtils.Platform.Windows &&
	...
	private async spawnChromeUnelevatedWithWindowsScriptHost(
		...
		const chromeProc = fork(
			getChromeSpawnHelperPath(),
			[
				`${process.env.windir}\\System32\\cscript.exe`,
				path.join(__dirname, "launchUnelevated.js"),
				semaphoreFile,
				chromePath,
				...chromeArgs,
			],
...
async function findNewlyLaunchedChromeProcess(
	...
	for (let i = 0; i < 25; i++) {
		if (fs.existsSync(semaphoreFile)) {
			lastAccessFileContent = fs
				.readFileSync(semaphoreFile, {
					encoding: "utf16le",
				})
				.toString();
