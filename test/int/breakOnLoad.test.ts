...
function runCommonTests(breakOnLoadStrategy: string) {
	...
	setup(() => {
		return testSetup
			.setup(undefined, { breakOnLoadStrategy: breakOnLoadStrategy })
			.then((_dc) => (dc = _dc));
	});
	...
	// the breakpoint is not yet 'hit' and the reason is given as 'debugger_statement'
	...
	suite("TypeScript Project with SourceMaps", () => {
		...
		test("Hits a breakpoint at (1,1) in a file on load", async () => {
			...
			if (breakOnLoadStrategy === "instrument") {
				await launchWithUrlAndSetBreakpoints(
					url,
					testProjectRoot,
					scriptPath,
					bpLine,
...
suite("BreakOnLoad", () => {
	...
	suite("Instrument Webpack Project", () => {
		...
		setup(() => {
			return testSetup
				.setup(undefined, { breakOnLoadStrategy: "instrument" })
				.then((_dc) => (dc = _dc));
		});
	...
	suite("BreakOnLoad Disabled (strategy: off)", () => {
		...
		setup(() => {
			return testSetup
				.setup(undefined, { breakOnLoadStrategy: "off" })
				.then((_dc) => (dc = _dc));
		});
			...
			return new Promise((resolve, reject) => {
				...
				dc.addListener("output", function (event) {
					...
					Promise.all([
						...
						dc.launch({
							url: "http://localhost:7890/index.html",
