...
import {
	...
	IMockChromeConnectionAPI,
} from "./debugProtocolMocks";
...
suite("ChromeDebugAdapter", () => {
	...
	setup(() => {
		...
		const cDAClass: typeof _ChromeDebugAdapter =
			require(MODULE_UNDER_TEST).ChromeDebugAdapter;
		chromeDebugAdapter = new cDAClass(
			{
				chromeConnection: function () {
					return mockChromeConnection.object;
				},
			} as any,
			mockChromeDebugSession.object as any
		);
	});
	...
	suite("launch()", () => {
		...
		setup(() => {
			...
			mockChrome.Runtime.setup((x) => x.evaluate(It.isAny())).returns(
				() =>
					Promise.resolve<any>({
			...
			mockChrome.Network.setup((x) =>
				x.setCacheDisabled(It.isAny())
			).returns(() => Promise.resolve());
		});
		...
		test("launches with minimal correct args", () => {
			...
			require("../src/utils").getBrowserPath = (version) => {
				return "c:\\someplace\\msedge.exe";
			};
			function fork(
				chromeSpawnHelperPath: string,
				[chromePath, ...args]: string[]
			): any {
				// Just assert that the chrome path is some string with 'chrome' in the path, and there are >0 args
				assert(
					chromeSpawnHelperPath.indexOf("chromeSpawnHelper.js") >= 0
				);
		...
		test("launches unelevated with client", async () => {
			let telemetryPropertyCollector =
				new telemetry.TelemetryPropertyCollector();
			...
			const originalGetBrowser = require("../src/utils").getBrowserPath;
			require("../src/utils").getBrowserPath = () => {
				return "c:\\someplace\\chrome.exe";
			};
			...
			assert.equal(
				expectedProcessId,
				(<any>chromeDebugAdapter)._chromePID,
				"Debug Adapter should receive the Chrome process id"
			);
			...
			assert(
				collectedLaunchParams.args.filter(
					(arg) => arg === "--no-default-browser-check"
				).length !== 0,
				"Should have seen the --no-default-browser-check parameter"
			);
			assert(
				collectedLaunchParams.args.filter(
					(arg) => arg === "--no-first-run"
				).length !== 0,
				"Should have seen the --no-first-run parameter"
			);
			assert(
				collectedLaunchParams.args.filter((arg) => arg === "abc")
					.length !== 0,
				"Should have seen the abc parameter"
			);
			assert(
				collectedLaunchParams.args.filter((arg) => arg === "def")
					.length !== 0,
			...
			assert(
				collectedLaunchParams.args.filter((arg) =>
					arg.match(/remote-debugging-port/)
				).length !== 0,
				"Should have seen a parameter like remote-debugging-port"
			);
			assert(
				collectedLaunchParams.args.filter((arg) =>
					arg.match(/user-data-dir/)
				).length !== 0,
			...
			assert.equal(
				telemetryProperties.shouldLaunchEdgeUnelevated,
				"true",
				"Should send telemetry that Chrome is requested to be launched unelevated.'"
			);
			assert.equal(
				telemetryProperties.doesHostSupportLaunchUnelevated,
				"true",
				"Should send telemetry that host supports launcheing Chrome unelevated.'"
			);
	...
	suite("resolveWebRootPattern", () => {
		...
		const resolveWebRootPattern =
			require(MODULE_UNDER_TEST).resolveWebRootPattern;
		...
		test("resolves the webRoot pattern", () => {
			assert.deepEqual(
				resolveWebRootPattern(WEBROOT, <ISourceMapPathOverrides>{
					"/src": "${webRoot}/app/src",
				}),
				{ "/src": WEBROOT + "/app/src" }
			);
		...
		test(`ignores the webRoot pattern when it's not at the beginning of the string`, () => {
			const overrides: ISourceMapPathOverrides = {
				"/another/${webRoot}/src": "/app/${webRoot}/src",
			};
			assert.deepEqual(
				resolveWebRootPattern(WEBROOT, overrides),
				overrides
