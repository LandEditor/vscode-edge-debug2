...
suite("Utils", () => {
	...
	suite("getTargetFilter()", () => {
		test("defaultTargetFilter", () => {
			...
			assert.deepEqual(targets.filter(defaultTargetFilter), [
				{ type: "page" },
			]);
		});
		...
		test("getTargetFilter", () => {
			...
			const targets = [{ type: "page" }, { type: "webview" }];
			assert.deepEqual(targets.filter(getTargetFilter(["page"])), [
				{ type: "page" },
			]);
			assert.deepEqual(targets.filter(getTargetFilter(["webview"])), [
				{ type: "webview" },
			]);
			assert.deepEqual(
				targets.filter(getTargetFilter(["page", "webview"])),
				targets
			);
