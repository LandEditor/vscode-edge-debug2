...
function getDebuggerStubs(mockEventEmitter) {
	return {
		...
		on(eventName, handler) {
			mockEventEmitter.on(`Debugger.${eventName}`, handler);
		},
	};
}
...
function getRuntimeStubs(mockEventEmitter) {
	return {
		...
		on(eventName, handler) {
			mockEventEmitter.on(`Runtime.${eventName}`, handler);
		},
	};
}
...
function getInspectorStubs(mockEventEmitter) {
	return {
		on(eventName, handler) {
			mockEventEmitter.on(`Inspector.${eventName}`, handler);
		},
	};
}
...
export function getMockChromeConnectionApi(): IMockChromeConnectionAPI {
	...
	const mockConsole = Mock.ofInstance<Crdp.ConsoleApi>(
		<any>getConsoleStubs()
	);
	mockConsole.callBase = true;
	mockConsole.setup((x) => x.enable()).returns(() => Promise.resolve());

	const mockDebugger = Mock.ofInstance<Crdp.DebuggerApi>(
		<any>getDebuggerStubs(mockEventEmitter)
	);
	mockDebugger.callBase = true;
	mockDebugger.setup((x) => x.enable()).returns(() => Promise.resolve(null));

	const mockNetwork = Mock.ofInstance<Crdp.NetworkApi>(
		<any>getNetworkStubs()
	);
	...
	const mockRuntime = Mock.ofInstance<Crdp.RuntimeApi>(
		<any>getRuntimeStubs(mockEventEmitter)
	);
	mockRuntime.callBase = true;
	mockRuntime.setup((x) => x.enable()).returns(() => Promise.resolve());

	const mockInspector = Mock.ofInstance<Crdp.InspectorApi>(
		<any>getInspectorStubs(mockEventEmitter)
	);
	mockInspector.callBase = true;

	const mockPage = Mock.ofInstance<Crdp.PageApi>(<any>getPageStubs());

	const mockBrowser = Mock.ofInstance<Crdp.BrowserApi>(
		<any>getBrowserStubs()
	);
