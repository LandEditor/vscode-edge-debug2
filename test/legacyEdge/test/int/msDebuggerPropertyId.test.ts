	...
	suite("Test setting variable values:", () => {
		test("Test updating local scope variable using Debugger.setVariableValue.", async () => {
			...
			await dc.setBreakpointsRequest({
				...
				breakpoints: [{ line: bpLine2, column: bpCol2 }],
			});
			...
			scopes = await dc.scopesRequest({
				frameId: st.body.stackFrames[0].id,
			});
			localsRef = scopes.body.scopes[0].variablesReference;
			variables = await dc.variablesRequest({
				variablesReference: localsRef,
			});
			...
			assert.equal(
				xChecked,
				true,
				"unable to confirm x was properly updated."
			);
			assert.equal(
				yChecked,
				true,
				"unable to confirm y was properly updated."
			);
		});
		...
		test("Test updating object scope variable using Debugger.msSetDebuggerPropertyId.", async () => {
			...
			let objRef = findTargetVariableReference(
			...
			objRef = findTargetVariableReference(
				variables.body.variables,
				"obj"
			);
			const objRef2 = findTargetVariableReference(
				variables.body.variables,
				"obj2"
			);
			...
			assert.equal(
				objChecked,
				true,
				"unable to confirm obj.b was properly updated."
			);
			// Ensure 'obj2.b' is updated
			let obj2Checked = false;
			variables = await dc.variablesRequest({
				variablesReference: objRef2,
			});
			for (const variable of variables.body.variables) {
				if (variable.name === "b") {
					assert.equal(variable.value, "999");
			...
			assert.equal(
				obj2Checked,
				true,
				"unable to confirm obj2.b was properly updated."
			);
		});
		...
		test("Test updating object scope accessor Property using Runtime.callFunctionOn.", async () => {
			...
			assert.equal(
				objChecked,
				true,
				"unable to confirm obj.accessorProp was properly updated."
			);
			...
			assert.equal(
				obj2Checked,
				true,
				"unable to confirm obj2.accessorProp was properly updated."
			);
		});
	});
});

function findTargetVariableReference(
