...
import {
	getMockEdgeConnectionApi,
	IMockEdgeConnectionAPI,
...
suite("EdgeDebugAdapter", () => {
	...
	setup(() => {
		...
		mockEdgeConnection.setup((x) => x.isAttached).returns(() => isAttached);
		mockEdgeConnection
		...
		mockEdgeConnection.setup((x) => x.onClose(It.isAny()));
		mockEdgeConnection
			.setup((x) => x.events)
		...
		const cDAClass: typeof _EdgeDebugAdapter =
			require(MODULE_UNDER_TEST).EdgeDebugAdapter;
		edgeDebugAdapter = new cDAClass(
			{
				chromeConnection: function () {
					return mockEdgeConnection.object;
				},
			} as any,
			new MockEdgeDebugSession() as any
		);
	});
	...
	suite("launch()", () => {
		...
		test("launches with minimal correct args", () => {
			...
			function fork(
				edgeSpawnHelperPath: string,
				[edgePath, ...args]: string[]
			): any {
			...
			mockEdge.Runtime.setup((x) => x.evaluate(It.isAny())).returns(() =>
				Promise.resolve<any>({
					result: { type: "string", value: "123" },
				})
			);

			return edgeDebugAdapter
				.launch({ file: "c:\\path with space\\index.html" })
	...
	suite("evaluate()", () => {
		...
		teardown(() => {
			...
			(<any>edgeDebugAdapter).chrome.Debugger =
				chromeDebuggerEvaluateOnCallFrame_beforeOverride;
			(<any>edgeDebugAdapter).remoteObjectToVariable =
				remoteObjectToVariable_beforeOverride;
		});

		test("returns error when watch variable is missing", () => {
			let callFrameId: number = 1;
			let evalArgs: DebugProtocol.EvaluateArguments = {
				expression: "zzz",
				frameId: callFrameId,
				context: "watch",
			};
			...
			chromeDebuggerEvaluateOnCallFrame_beforeOverride = (<any>(
				edgeDebugAdapter
			)).chrome.Debugger;
			remoteObjectToVariable_beforeOverride = (<any>edgeDebugAdapter)
				.remoteObjectToVariable;

			(<any>edgeDebugAdapter)._frameHandles = {
				get: () => ({ callFrameId }),
			};
			(<any>edgeDebugAdapter).chrome.Debugger = {
				evaluateOnCallFrame: () => ({
			...
			return edgeDebugAdapter.evaluate(evalArgs).then(
				...
				(err) => {
					assert.equal(
						err.message,
						"not available",
						"Error should contain 'not available' message."
					);
				}
			);
		});
